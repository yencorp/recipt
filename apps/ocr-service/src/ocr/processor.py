...
class OCRProcessor:
    ...
    def _load_image(self, image_data: bytes) -> Optional[Image.Image]:
        ...
        try:
            ...
            # 크기 제한
            if (
                image.size[0] > self.settings.MAX_IMAGE_WIDTH
                or image.size[1] > self.settings.MAX_IMAGE_HEIGHT
            ):
                image.thumbnail(
                    (self.settings.MAX_IMAGE_WIDTH, self.settings.MAX_IMAGE_HEIGHT),
    ...
    async def _extract_text(self, image: Image.Image) -> str:
        ...
        try:
            ...
            text = pytesseract.image_to_string(
                image, lang=self.settings.OCR_LANG, config=custom_config
            )
                ...
                for pattern in amount_patterns:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            # 가장 큰 금액을 총액으로 가정
                            if (
                                not structured["total_amount"]
                                or amount > structured["total_amount"]
                            ):
