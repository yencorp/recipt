...
export const SettlementIncomeList: React.FC<SettlementIncomeListProps> = ({
  items,
  onChange,
}) => {
  ...
  const totalBudgetIncome = items.reduce((sum, item) => sum + item.budgetAmount, 0);
  const totalActualIncome = items.reduce((sum, item) => sum + item.actualAmount, 0);
  const difference = totalActualIncome - totalBudgetIncome;
                    ...
                    {items.map((item, index) => {
                      ...
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell className="text-right">
                            <Badge variant={itemDiff >= 0 ? 'default' : 'secondary'}>
                              {itemDiff >= 0 ? '+' : ''}
                  ...
                  <div className="flex justify-end gap-8">
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold">
                        {totalBudgetIncome.toLocaleString()}원
                      </p>
                    </div>
                    <div className="text-right">
