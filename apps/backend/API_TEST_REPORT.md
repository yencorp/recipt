...
### 문제 상황
NestJS 통합 테스트(Integration/E2E) 환경 구성 중 다음과 같은 문제가 발생했습니다:

1. **Integration Tests (모듈별 테스트)**
   - 문제: TypeORM repository 초기화 실패
   - 에러: `TypeError: Cannot read properties of undefined (reading 'find')`
   - 원인: 복수 모듈 import 시 TypeORM 설정 충돌
   - 파일: `src/modules/*/*.integration-spec.ts`

2. **E2E Tests (전체 앱 테스트)**
   - 문제: 모든 API 엔드포인트에서 404 응답
   - 현상: TypeORM은 정상 초기화되나 라우트가 등록되지 않음
   - 시도한 해결책: `app.setGlobalPrefix("api")` 추가 → 효과 없음
   - 파일: `test/auth.e2e-spec.ts`

### 임시 해결 방안
현재는 **실제 서버를 구동한 상태에서 supertest로 API 테스트**를 수행하는 방식을 사용합니다.
- 서버 구동: `docker-compose up -d`
- API 테스트: REST Client 또는 curl 사용
- 장점: 실제 프로덕션 환경과 동일한 조건에서 테스트
- 단점: 테스트 격리가 어렵고 데이터 정리 필요

### 향후 계획
- [ ] NestJS 공식 문서의 Testing 가이드 재검토
- [ ] 다른 NestJS 프로젝트의 테스트 설정 참고
- [ ] TestingModule 설정 방법 재구성
- [ ] 또는 현재 방식(실서버 테스트) 유지 및 체계화
